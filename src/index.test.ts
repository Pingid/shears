...
const mocNetwork = (html: string, fn?: (x: any, c: any) => void) => <T>(url: string, last: T) => {
  if (fn) fn(url, last)
  return TE.tryCatch(
...
it('Should return first node matching a css selector', async () => {
  const fetch = mocNetwork(`<div><h1>hello</h1><h2>world</h2><h2>something</h2></div>`)
...
it('Should join multipe shears together', async () => {
  const fetch = mocNetwork(`<div><ul><li><h1>foo</h1></li><li></li></ul></div>`)
  const result = await run(fetch, undefined, goTo('', join($('ul'), $('li'), $('h1'))))()
  if (isLeft(result)) throw result.left
...
it('Should return sibling nodes', async () => {
  ...
  const result = await run(fetch, undefined, goTo('', join($('h1'), siblings)))()
  if (isLeft(result)) throw result.left
...
it('Should return next node sibling', async () => {
  const fetch = mocNetwork(`<div><h1>one</h1><h2>two</h2><h3>three</h3></div>`)
  const result = await run(fetch, undefined, goTo('', join($('h1'), nextSibling)))()
  if (isLeft(result)) throw result.left
...
it('Should return all node attributes', async () => {
  const fetch = mocNetwork(`<div style="background:blue" role="button">click me</div>`)
  const result = await run(fetch, undefined, goTo('', join($('div'), attributes)))()
  if (isLeft(result)) throw result.left
...
it('Should return single node attribute', async () => {
  const fetch = mocNetwork(`<div style="background:blue" role="button">click me</div>`)
  const result = await run(fetch, undefined, goTo('', join($('div'), attr('style'))))()
  if (isLeft(result)) throw result.left
...
it('Should resolve object structured shears selectors', async () => {
  ...
  const result = await run(fetch, undefined, goTo('', fork({ h1: join($('h1'), text), h2: join($('h2'), text) })))()
  if (isLeft(result)) throw result.left
...
it('Should resolve array structured shears selectors', async () => {
  const fetch = mocNetwork(`<div><h1>foo</h1><h2>bar</h2></div>`)
  const result = await run(fetch, undefined, goTo('', fork([join($('h1'), text), join($('h2'), text)])))()
  if (isLeft(result)) throw result.left
  expect(result.right).toEqual(['foo', 'bar'])
})

it('Should run shears selector on array of inputs', async () => {
  const fetch = mocNetwork(`<ul><li><h1>one</h1></li><li><h1>two</h1></li><li><h1>three</h1></li></ul>`)
  const result = await run(fetch, undefined, goTo('', join($$('li'), each(join($('h1'), text)))))()
  if (isLeft(result)) throw result.left
